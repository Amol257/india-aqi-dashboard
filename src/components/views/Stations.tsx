  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#181c22]">Stations Analysis</h1>
          <p className="text-[#414753] mt-2 max-w-2xl font-medium tracking-tight">
            Real-time telemetry and distribution analysis of {activeCount} active monitoring sites.
          </p>
        </div>
        <div className="flex items-center gap-3 bg-white p-1 rounded-xl border border-slate-200 shadow-sm self-start group transition-all hover:border-[#1275e2]">
          <div className="bg-[#1275e2] text-white p-2 rounded-lg group-hover:scale-110 transition-transform">
             <Radio size={16} />
          </div>
          <span className="text-xs font-black text-[#181c22] pr-5 uppercase tracking-widest">{activeCount} Active Sites</span>
        </div>
      </section>

      {/* Filter Dock */}
      <div className="bg-white/80 backdrop-blur-xl border border-white rounded-3xl p-3 md:p-1.5 flex flex-col md:flex-row items-center gap-6 shadow-lg shadow-[#ebedf7]/50 sticky top-20 z-30 transition-all hover:shadow-2xl">
        <div className="flex items-center gap-4 w-full md:w-auto px-4 md:border-r border-[#c1c6d5]/30">
          <span className="text-[10px] font-black text-[#717785] uppercase tracking-widest leading-none">Region</span>
          <select 
            value={region}
        ...
        <div className="flex items-center gap-4 w-full md:w-auto px-4 md:border-r border-[#c1c6d5]/30">
          <span className="text-[10px] font-black text-[#717785] uppercase tracking-widest leading-none">Sort</span>
          <select 
            value={sortOrder}
          ...
          >
            ...
            <option value="asc">AQI (Low to High)</option>
            <option value="desc">AQI (High to Low)</option>
          </select>
        </div>
        ...
        <div className="flex flex-1 items-center gap-3 px-4 w-full group">
          ...
          <input 
            ...
            className="bg-transparent border-none outline-none text-sm font-bold w-full placeholder:text-[#c1c6d5]"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </div>

        <button className="w-full md:w-auto bg-[#1275e2] text-white px-8 py-3 rounded-2xl flex items-center justify-center gap-2 font-black text-[11px] uppercase tracking-[0.2em] shadow-lg shadow-blue-100 hover:scale-[1.02] transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        <div className="md:col-span-8 bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h3 className="text-xl font-black text-[#181c22] tracking-tight">Station Distribution</h3>
              <p className="text-xs font-bold text-[#717785] uppercase tracking-widest mt-1">Network density across major clusters</p>
            </div>
            <Map size={24} className="text-[#1275e2]" />
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {[
              ...
              >
                ...
                <span className="text-[9px] font-bold text-slate-300 ml-1">{item.count}</span>
              </button>
            ))}
          </div>

          <div className="h-[320px] w-full rounded-2xl bg-[#ebedf7]/40 relative overflow-hidden group shadow-inner border border-[#c1c6d5]/10 z-0">
            <MapContainer 
        ...
        <div className="md:col-span-4 bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-10">AQI Range Histogram</h3>
          ...
          <div className="flex justify-between mt-4 px-2">
            <span className="text-[10px] font-black text-slate-400">GOOD</span>
            <span className="text-[10px] font-black text-slate-400">SEVERE</span>
          </div>
        </div>

        <div className="md:col-span-3 bg-[#0a73e0] text-white rounded-3xl p-8 shadow-xl shadow-blue-100 flex flex-col justify-between group">
          <div>
        ...
        <div className="md:col-span-3 bg-[#ebedf7] rounded-3xl p-8 border border-[#c1c6d5]/20 flex flex-col justify-between">
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Avg Latency</p>
            <div className="text-5xl font-black text-[#1275e2] mt-2 tracking-tighter">1.2s</div>
          </div>
          <div className="flex items-baseline gap-1.5 mt-6 h-10">
            {[30, 50, 40, 90].map((h, i) => (
              <div key={i} className="flex-1 bg-[#1275e2]/30 rounded-t-sm" style={{ height: `${h}%` }} />
            ))}
          </div>
        </div>

        <div className="md:col-span-6 bg-white rounded-3xl p-8 shadow-sm border border-slate-100 relative overflow-hidden group">
          <div className="relative z-10">
            <h4 className="text-lg font-black tracking-tight mb-6">System Health Status</h4>
            <div className="grid grid-cols-3 gap-8">
              {[
                { l: 'Good AQI', v: goodAqiCount, c: 'text-emerald-500' },
                { l: 'Medium AQI', v: mediumAqiCount, c: 'text-amber-500' },
                { l: 'High AQI', v: highAqiCount, c: 'text-red-500' },
              ].map(i => (
                <div key={i.l}>
                  <div className={cn("text-3xl font-black", i.c)}>{i.v}</div>
                  <div className="text-[10px] font-black text-slate-400 uppercase mt-1 tracking-widest">{i.l}</div>
                </div>
              ))}
            </div>
          </div>
          <Settings2 size={160} className="absolute -bottom-8 -right-8 text-[#1275e2] opacity-[0.03] group-hover:rotate-12 transition-transform duration-1000" />
        </div>
      </div>
      ...
      <section className="bg-white rounded-4xl border border-slate-100 mt-12 overflow-hidden shadow-sm">
        <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 px-4 border-l border-slate-200">
              ...
              <select 
                ...
                className="bg-white border border-slate-200 rounded-lg text-xs font-black px-2 py-1 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer"
              >
          ...
          <div className="flex gap-2">
            <button className="px-4 py-2 border rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all">CSV</button>
            <button className="px-4 py-2 bg-[#1275e2] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-50">+ Add</button>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {paginatedStations.length > 0 ? paginatedStations.map(s => (
                <tr key={s.id} className="hover:bg-slate-50 transition-colors group cursor-pointer">
                  <td className="px-6 py-4 font-mono text-xs font-black text-slate-400 tracking-tighter">#{s.id}</td>
                  <td className="px-6 py-4">
                    <p className="font-black text-sm text-[#181c22]">{s.location}</p>
                    <p className="text-[10px] font-bold text-slate-400 mt-0.5">{s.city}</p>
                  </td>
                  <td className="px-6 py-4 flex gap-1 pt-6">
                    {s.pollutants.map(p => <span key={p} className="px-2 py-0.5 bg-blue-50 text-[#1275e2] text-[9px] font-black rounded-md">{p}</span>)}
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full shadow-sm" style={{ backgroundColor: getAqiColor(s.aqi) }} />
                      <span className="font-black text-lg tracking-tighter">{s.aqi}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={cn("px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest", s.status === 'ACTIVE' ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600")}>{s.status}</span>
                  </td>
                </tr>
              )) : (
                <tr>
        ...
        {totalPages > 1 && (
          <div className="px-8 py-6 bg-slate-50/50 border-t border-slate-50 flex items-center justify-between">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
              Showing <span className="text-slate-900">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-slate-900">{Math.min(currentPage * itemsPerPage, filteredStations.length)}</span> of <span className="text-slate-900">{filteredStations.length}</span> stations
            </p>
            <div className="flex items-center gap-2">
              <button 
                onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))}
                disabled={currentPage === 1}
                className="p-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-all disabled:opacity-30 disabled:cursor-not-allowed group"
              >
                <Radio size={16} className="rotate-90 text-slate-400 group-hover:text-blue-500" />
              </button>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        currentPage === pageNum 
                          ...
                          : "border border-slate-200 bg-white text-slate-400 hover:bg-slate-50 hover:text-slate-900"
                      )}
                    >
                      {pageNum}
                    </button>
